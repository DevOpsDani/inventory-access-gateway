# Inventory Access Gateway - High-Level Architecture
...
flowchart LR
    A[Developer commits code
     to app repository] --> B[CI builds Docker image]
    B --> C[Run security scans]
    C --> D[Push approved image]
    D --> E[Update image tag in 
    dev\test GitOps repo]
    E --> F[ArgoCD syncs
    deploys to lower envs]
    F --> G[Integration and QA tests run]
    G --> H[Promote same image tag 
    to Staging GitOps]
    H --> I[ArgoCD syncs and deploys 
    to Staging cluster]
    I --> J[Manual approval or 
    automated promotion to Prod]
    J --> K[Update GitOps Prod environment]
    K --> L[ArgoCD syncs and deploys to Prod cluster]
```
